...
export default function ProductItem({product}) {
    ...
    return (
        <div
            className="relative max-w-sm bg-white rounded-xl border border-gray-100 overflow-hidden flex flex-col transition-transform duration-300 hover:scale-105 hover:shadow-md my-4">
            {discountPercentage > 0 && (
                <div
                    className="absolute top-2 right-2 bg-red-600 text-white text-sm font-bold py-1 px-3 rounded-full shadow-md">
                    -{discountPercentage}%
                </div>
            )}
            ...
            <div className="p-4 flex flex-col flex-grow">
                <div
                    ...
                    style={{height: "60px", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical"}}
                    onClick={() => window.open(product.link, "_blank")}
                >
                    {product.name}
                </div>
